module Sketchup
  ...
  # interface is often used to attach other observers to models as they are
  # opened or started. This ensures that your observers are watching all open
  # models.
  #
  # For example, when one attaches a SelectionObserver, it is only attached to
  # the Selection collection of a given model. If a 2nd model is opened, the new
  ...
  # desired methods, and add an instance of the observer to the application
  # class.
  #
  # @example
  ...
  class AppObserver
    ...
    # line arguments, or auto-restored models on Mac OS X.
    #
    # @note Prior to SketchUp 2014, these methods were not being called for the
    #   startup models. This issue is now fixed but observers still need to
    #   express their intent to receive these calls. This is for
    #   back-compatibility with existing scripts which worked around these
    #   missing calls by other means. For new code, this method should be
    #   implemented and should return true.
    #
    # @example
    ...
    #   skp in explorer (which is also is the command line) then this observer
    #   will not be called. The Ruby interpreter in SketchUp is initialized
    #   after command line processing so the observer won't be added in time to
